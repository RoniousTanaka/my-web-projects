...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_internship'])) {
    ...
    if (empty($title) || empty($company)) {
        ...
        if (isset($_POST['internship_id']) && !empty($_POST['internship_id'])) {
            ...
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssssdsss", $title, $company, $description, $required_skills, $min_gpa, $location, $industry, $deadline);
    ...
    <div class="container">
        ...
        <div class="card">
            ...
            <form method="POST" action="internships.php">
                ...
                <div class="form-group">
    <label for="skills_input_method">Skills Selection Method:</label>
    <select id="skills_input_method" name="skills_input_method" class="form-control" onchange="toggleSkillsInput()">
        <option value="text">Enter as text</option>
        <option value="checkboxes">Select from list</option>
    </select>
</div>

<div id="text_skills_input" class="form-group">
...
<div id="checkbox_skills_input" class="form-group" style="display:none;">
    <label>Select Skills:</label>
    <div class="skills-checkboxes">
        <?php 
        $commonSkills = ['PHP', 'JavaScript', 'Python', 'Java', 'SQL', 'HTML/CSS', 'React', 'Node.js', 
                        'Network Security', 'Data Analysis', 'Machine Learning', 'Project Management'];
        foreach ($commonSkills as $skill): ?>
            <div class="skill-checkbox">
                <input type="checkbox" id="skill_<?php echo strtolower(str_replace(' ', '_', $skill)); ?>" 
                       name="skills[]" value="<?php echo $skill; ?>">
                <label for="skill_<?php echo strtolower(str_replace(' ', '_', $skill)); ?>"><?php echo $skill; ?></label>
            </div>
        <?php endforeach; ?>
    </div>
</div>
...
function toggleSkillsInput() {
    const method = document.getElementById('skills_input_method').value;
    document.getElementById('text_skills_input').style.display = method === 'text' ? 'block' : 'none';
    document.getElementById('checkbox_skills_input').style.display = method === 'checkboxes' ? 'block' : 'none';
}
</script>
                <div class="form-row">
                    <div class="form-col">
